...
// Thai text wrapper to help with text rendering
const ThaiText = ({ children, style }: { children: React.ReactNode; style?: any }) => (
    <Text style={style}>{children}</Text>
);

// Styles using built-in fonts only (reliable and stable)
        ...
        <Page size="A4" style={styles.page}>
            ...
            <View style={styles.table}>
                <View style={styles.tableHeader}>
                    <ThaiText style={styles.col1}>รายการสินค้า / Item Description</ThaiText>
                    <ThaiText style={styles.col2}>รหัสสินค้า / Code</ThaiText>
                    <ThaiText style={styles.col3}>จำนวน / Qty</ThaiText>
                    <ThaiText style={styles.col4}>หน่วย / Unit</ThaiText>
                    <ThaiText style={styles.col5}>ราคาต่อหน่วย / Unit Price</ThaiText>
                    <ThaiText style={styles.col6}>ราคารวม / Total</ThaiText>
                </View>
                {data.items.map((item, index) => (
                    <View key={index} style={[styles.tableRow, index % 2 === 0 ? styles.tableRowEven : {}]}>
                        <ThaiText style={styles.col1}>{item.name}</ThaiText>
                        <ThaiText style={styles.col2}>{item.code}</ThaiText>
                        <ThaiText style={styles.col3}>{item.quantity}</ThaiText>
